...
export const locationTransferRouter = router({
      ...
      return await ctx.prisma.$transaction(async (tx) => {
        ...
        for (const item of input.items) {
          if (item.itemType === 'DEVICE') {
            ...
            const device = await tx.warehouse.findUniqueOrThrow({
            ...
            const stock = await tx.warehouse.findFirstOrThrow({
              where: {
  ...
  getIncomingLocationTransfers: adminCoordOrWarehouse
    ...
    .query(({ ctx, input }) => {
      ...
      return ctx.prisma.locationTransfer.findMany({
        where: {
          status: 'REQUESTED',
        ...
        for (const line of transfer.lines) {
          ...
          if (
            ...
            if (existing) {
              ...
              const newRow = await tx.warehouse.create({
                data: {
                  itemType: 'MATERIAL',
