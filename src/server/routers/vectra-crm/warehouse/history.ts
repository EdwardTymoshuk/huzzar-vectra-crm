...
import { VectraWarehouseAction, WarehouseItemType } from '@prisma/client'
...
export const historyRouter = router({
  ...
  getHistory: loggedInEveryone
    ...
    .query(async ({ input, ctx }) => {
      ...
      return prisma.vectraWarehouseHistory.findMany({
        where,
        include: {
          performedBy: true,
          assignedTo: {
  ...
  getHistoryByName: loggedInEveryone
    ...
    .query(async ({ input, ctx }) => {
      ...
      const rows = await prisma.vectraWarehouseHistory.findMany({
        where: {
          warehouseItem: {
