// src/server/modules/opl-crm/services/orderMaterialsReconciliation.ts
...
export async function reconcileOrderMaterials({
  ...
  if (!technicianId) {
    ...
    await tx.oplOrderMaterial.deleteMany({ where: { orderId } })
    return { warnings }
  }
  ...
  // -----------------------------------------------------------
  const previous = await tx.oplOrderMaterial.findMany({
    where: { orderId },
  ...
  for (const prev of previous) {
    ...
    const techItem = await tx.oplWarehouse.findFirst({
      where: {
    ...
    const deficit = await tx.oplTechnicianMaterialDeficit.findUnique({
      where: {
    ...
    if (deficit && deficit.quantity > 0) {
      ...
      await tx.oplTechnicianMaterialDeficit.update({
        where: {
          technicianId_materialDefinitionId: {
            technicianId,
            materialDefinitionId: prev.materialId,
  ...
  if (newMaterials.length > 0) {
    const defs = await tx.oplMaterialDefinition.findMany({
      where: { id: { in: newMaterials.map((m) => m.id) } },
      select: { id: true, unit: true },
    })
    const unitMap = new Map(defs.map((d) => [d.id, d.unit]))

    await tx.oplOrderMaterial.createMany({
      data: newMaterials.map((m) => ({
        orderId,
        materialId: m.id,
        quantity: m.quantity,
        unit: (unitMap.get(m.id) as OplMaterialUnit) ?? OplMaterialUnit.PIECE,
      })),
    })
  }
  ...
  for (const item of newMaterials) {
    ...
    if (missing > 0) {
      await tx.oplTechnicianMaterialDeficit.upsert({
        where: {
          technicianId_materialDefinitionId: {
            technicianId,
            materialDefinitionId: item.id,
