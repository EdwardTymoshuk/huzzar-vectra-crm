//src/server/modules/opl-crm/services/orderEquipmentDelta.ts
...
export async function processEquipmentDelta({
   ...
   * ------------------------------------------------------------------- */
  const previous: PreviousDevice[] = await tx.oplWarehouse.findMany({
    where: {
      itemType: 'DEVICE',
      orderAssignments: { some: { orderId } },
      status: { not: OplWarehouseStatus.COLLECTED_FROM_CLIENT },
    },
    select: { id: true },
  })
