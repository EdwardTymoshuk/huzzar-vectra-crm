//scr/server/modules/opl-crm/routers/warehouse/locationTransfer.ts
...
export const locationTransferRouter = router({
  ...
  createTransfer: adminCoordOrWarehouse
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      return await ctx.prisma.$transaction(async (tx) => {
        const transfer = await tx.oplLocationTransfer.create({
          data: {
            fromLocationId,
        ...
        for (const item of input.items) {
          if (item.itemType === 'DEVICE') {
            ...
            const device = await tx.oplWarehouse.findUniqueOrThrow({
            ...
            await tx.oplLocationTransferLine.create({
              data: {
            ...
            const stock = await tx.oplWarehouse.findFirstOrThrow({
              where: {
  ...
  getIncomingLocationTransfers: adminCoordOrWarehouse
    ...
    .query(({ ctx, input }) => {
      ...
      return ctx.prisma.oplLocationTransfer.findMany({
        where: {
          status: 'REQUESTED',
        ...
        include: {
          ...
          fromLocation: true,
          toLocation: true,
          requestedBy: { select: oplUserBasicSelect },
        },
    ...
    .query(({ ctx, input }) => {
      ...
      return ctx.prisma.oplLocationTransfer.findMany({
        ...
        include: {
          ...
          toLocation: true,
          fromLocation: true,
          confirmedBy: { select: oplUserBasicSelect },
        },
        orderBy: { createdAt: 'desc' },
      })
    }),
  ...
  confirmLocationTransfer: adminCoordOrWarehouse
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      await ctx.prisma.$transaction(async (tx) => {
        const transfer = await tx.oplLocationTransfer.findUniqueOrThrow({
        ...
        for (const line of transfer.lines) {
          ...
          if (
            ...
            const existing = await tx.oplWarehouse.findFirst({
              where: {
                locationId: transfer.toLocationId,
            ...
            if (existing) {
              ...
              const newRow = await tx.oplWarehouse.create({
                data: {
                  itemType: 'MATERIAL',
        ...
        await tx.oplLocationTransfer.update({
  ...
  cancelLocationTransfer: adminCoordOrWarehouse
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      await ctx.prisma.$transaction(async (tx) => {
        ...
        await tx.oplLocationTransferLine.deleteMany({
