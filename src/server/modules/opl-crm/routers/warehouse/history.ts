...
import { OplWarehouseAction, Prisma } from '@prisma/client'
import { z } from 'zod'
...
export const historyRouter = router({
   ...
   */
  getOplHistory: loggedInEveryone
    .input(
      z.object({
    ...
    .query(async ({ input, ctx }) => {
      const rows = await ctx.prisma.oplWarehouseHistory.findMany({
        where: {
  ...
  getOplHistoryByName: loggedInEveryone
    .input(
      z.object({
        name: z.string(),
      })
  ...
  getOplWarehouseHistory: adminCoordOrWarehouse
    .input(
      z.object({
        page: z.number().min(1).default(1),
        limit: z.number().min(1).max(100).default(30),
        actions: z.array(z.nativeEnum(OplWarehouseAction)).optional(),
    ...
    .query(async ({ input }) => {
      ...
      const whereBase: Prisma.OplWarehouseHistoryWhereInput = {
        action: actions?.length
          ? {
              in: actions.filter(
                (a) => !['ISSUED_TO_CLIENT', 'ASSIGNED_TO_ORDER'].includes(a)
              ) as OplWarehouseAction[],
            }
          : {
              notIn: [
                OplWarehouseAction.ISSUED_TO_CLIENT,
                OplWarehouseAction.ASSIGNED_TO_ORDER,
              ],
            },
        NOT: {
          AND: [
       ...
       */
      const rawGroups = await prisma.oplWarehouseHistory.findMany({
        where: whereBase,
        select: {
          performedById: true,
          action: true,
      ...
      const data = await Promise.all(
        paginatedKeys.map(async (key) => {
          ...
          const entries = await prisma.oplWarehouseHistory.findMany({
            where: {
              performedById: performerId,
              action: action as OplWarehouseAction,
