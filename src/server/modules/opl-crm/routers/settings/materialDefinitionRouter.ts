...
export const materialDefinitionRouter = router({
  ...
  createOplMaterialDefinition: adminOrCoord
    .input(materialSchema)
    .mutation(({ input }) => {
      return prisma.oplMaterialDefinition.create({
        data: {
  ...
  editOplMaterialDefinition: adminOrCoord
    .input(
      z
        .object({
          id: z.string(),
    ...
    .mutation(({ input }) => {
      return prisma.oplMaterialDefinition.update({
        where: { id: input.id },
        data: {
          name: input.name.trim(),
          index: input.index.trim(),
  ...
  deleteOplMaterialDefinition: adminOrCoord
    .input(z.object({ id: z.string() }))
    .mutation(({ input }) => {
      return prisma.oplMaterialDefinition.delete({
        where: { id: input.id },
      })
    }),
})
