...
import { vectraUserSlimSelect } from './../../helpers/selects'
...
export const queriesRouter = router({
    ...
    .input(
      z
        .object({
          ...
          itemType: z.nativeEnum(VectraWarehouseItemType).optional(),
        })
        .optional()
    )
  ...
  getDefinitionsWithStock: loggedInEveryone
    .input(
      z.object({
        itemType: z.nativeEnum(VectraWarehouseItemType),
        locationId: z.string().optional(),
      })
    )
    .query(async ({ ctx, input }) => {
  ...
  getItemsByName: loggedInEveryone
    ...
    .query(async ({ input, ctx }) => {
      ...
      const items = await prisma.vectraWarehouse.findMany({
        where: whereClause,
        select: {
          id: true,
          name: true,
          ...
          history: {
            select: {
              ...
              performedBy: { select: vectraUserSlimSelect },
              assignedTo: {
  ...
  getTechnicianStock: loggedInEveryone
    ...
    .query(async ({ input, ctx }) => {
      ...
      const rows = await prisma.vectraWarehouse.findMany({
        where: {
        ...
        include: {
          ...
          history: {
            select: {
              ...
              quantity: true,
              assignedOrderId: true,
              performedBy: { select: vectraUserBasicSelect },
  ...
  searchDevices: adminOrCoord
    .input(
      z.object({
        ...
        allowedCategories: z
          .nativeEnum(VectraDeviceCategory)
          .array()
          .optional(),
      })
    )
  ...
  checkExistingIdentifiers: adminCoordOrWarehouse
    ...
    .query(async ({ input, ctx }) => {
      ...
      const details: Record<
        string,
        { name: string; status: VectraWarehouseStatus; assignedTo?: string }
      > = {}
  ...
  getTechnicianDeficits: adminCoordOrWarehouse
    ...
    .query(async ({ input, ctx }) => {
      ...
      const deficits = await prisma.vectraTechnicianMaterialDeficit.findMany({
  ...
  getMaterialDeficitsSummary: adminCoordOrWarehouse.query(async () => {
    const rows = await prisma.vectraTechnicianMaterialDeficit.groupBy({
    ...
    const defs =
      ids.length > 0
        ? await prisma.vectraMaterialDefinition.findMany({
            where: { id: { in: ids } },
