...
import { VectraDeviceCategory, VectraWarehouseItemType } from '@prisma/client'
...
export const mutationsRouter = router({
  ...
  addItems: adminCoordOrWarehouse
    .input(
      z.object({
        items: z.array(
          z.object({
            type: z.nativeEnum(VectraWarehouseItemType),
            name: z.string(),
            category: z.nativeEnum(VectraDeviceCategory).optional(),
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      const vectraUser = await getVectraUserOrThrow(ctx.prisma, coreUser.id)

      const activeLocationId = resolveLocationId(vectraUser, input.locationId)

      const userId = vectraUser.userId

      for (const item of input.items) {
        if (item.type === 'DEVICE') {
          const def = await prisma.vectraDeviceDefinition.findFirst({
          ...
          if (normalizedSerial) {
            const existingDevice = await prisma.vectraWarehouse.findUnique({
            ...
            if (existingDevice) {
              ...
              await prisma.vectraWarehouse.update({
              ...
              await prisma.vectraWarehouseHistory.create({
                data: {
          ...
          const created = await prisma.vectraWarehouse.create({
            data: {
              itemType: 'DEVICE',
              name: item.name,
              category: item.category ?? 'OTHER',
        ...
        if (item.type === 'MATERIAL') {
          const def = await prisma.vectraMaterialDefinition.findFirst({
  ...
  issueItems: adminCoordOrWarehouse
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      for (const item of input.items) {
        ...
        if (item.type === 'MATERIAL') {
          const original = await prisma.vectraWarehouse.findUnique({
            where: { id: item.id },
          })
          ...
          const deficit =
            await prisma.vectraTechnicianMaterialDeficit.findUnique({
              where: {
          ...
          if (usedToCoverDeficit > 0) {
            if (deficitQty - usedToCoverDeficit > 0) {
              await prisma.vectraTechnicianMaterialDeficit.update({
                where: { id: deficit!.id },
                data: { quantity: deficitQty - usedToCoverDeficit },
              })
            } else {
              await prisma.vectraTechnicianMaterialDeficit.delete({
                where: { id: deficit!.id },
              })
            }
          }
          ...
          if (remainingToAddToStock > 0) {
            const existingTechMaterial = await prisma.vectraWarehouse.findFirst(
              {
                where: {
                  itemType: 'MATERIAL',
                  assignedToId: input.assignedToId,
            ...
            if (existingTechMaterial) {
              ...
              await prisma.vectraWarehouse.create({
                data: {
                  itemType: 'MATERIAL',
                  name: original.name,
  ...
  returnToWarehouse: adminCoordOrWarehouse
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      for (const item of input.items) {
        if (item.type === 'DEVICE') {
          const current = await prisma.vectraWarehouse.findUnique({
            where: { id: item.id },
        ...
        if (item.type === 'MATERIAL') {
          ...
          const existingStock = await prisma.vectraWarehouse.findFirst({
            where: {
              itemType: 'MATERIAL',
              name: original.name,
              index: original.index,
  ...
  returnToOperator: adminCoordOrWarehouse
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      for (const item of input.items) {
        ...
        const previous = await prisma.vectraWarehouseHistory.findFirst({
          where: {
            warehouseItemId: item.id,
        ...
        if (item.type === 'DEVICE') {
          const hist = await prisma.vectraWarehouseHistory.create({
            data: {
              warehouseItemId: item.id,
  ...
  collectFromClient: loggedInEveryone
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const device = await prisma.vectraWarehouse.create({
        data: {
          itemType: 'DEVICE',
      ...
      await prisma.vectraOrderEquipment.create({
