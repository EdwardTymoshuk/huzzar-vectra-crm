  ...
  createTransfer: adminCoordOrWarehouse
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      return await ctx.prisma.$transaction(async (tx) => {
        const transfer = await tx.vectraLocationTransfer.create({
          data: {
            fromLocationId,
        ...
        for (const item of input.items) {
          if (item.itemType === 'DEVICE') {
            ...
            await tx.vectraLocationTransferLine.create({
              data: {
    ...
    .query(({ ctx, input }) => {
      ...
      return ctx.prisma.vectraLocationTransfer.findMany({
        ...
        include: {
          ...
          fromLocation: true,
          toLocation: true,
          requestedBy: { select: vectraUserBasicSelect },
        },
    ...
    .query(({ ctx, input }) => {
      ...
      return ctx.prisma.vectraLocationTransfer.findMany({
        ...
        include: {
          ...
          toLocation: true,
          fromLocation: true,
          confirmedBy: { select: vectraUserBasicSelect },
        },
        orderBy: { createdAt: 'desc' },
      })
    }),
  ...
  confirmLocationTransfer: adminCoordOrWarehouse
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      await ctx.prisma.$transaction(async (tx) => {
        const transfer = await tx.vectraLocationTransfer.findUniqueOrThrow({
        ...
        await tx.vectraLocationTransfer.update({
  ...
  cancelLocationTransfer: adminCoordOrWarehouse
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      await ctx.prisma.$transaction(async (tx) => {
        ...
        await tx.vectraLocationTransferLine.deleteMany({
