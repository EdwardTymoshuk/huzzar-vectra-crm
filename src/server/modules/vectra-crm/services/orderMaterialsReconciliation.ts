// src/server/modules/vectra-crm/services/orderMaterialsReconciliation.ts
...
export async function reconcileOrderMaterials({
  ...
  if (!technicianId) {
    ...
    await tx.vectraOrderMaterial.deleteMany({ where: { orderId } })
    return { warnings }
  }
  ...
  // -----------------------------------------------------------
  const previous = await tx.vectraOrderMaterial.findMany({
    where: { orderId },
  ...
  for (const prev of previous) {
    ...
    const deficit = await tx.vectraTechnicianMaterialDeficit.findUnique({
      where: {
    ...
    if (deficit && deficit.quantity > 0) {
      ...
      await tx.vectraTechnicianMaterialDeficit.update({
        where: {
          technicianId_materialDefinitionId: {
            technicianId,
            materialDefinitionId: prev.materialId,
  ...
  if (newMaterials.length > 0) {
    const defs = await tx.vectraMaterialDefinition.findMany({
      where: { id: { in: newMaterials.map((m) => m.id) } },
      select: { id: true, unit: true },
    })
    const unitMap = new Map(defs.map((d) => [d.id, d.unit]))

    await tx.vectraOrderMaterial.createMany({
      data: newMaterials.map((m) => ({
        orderId,
        materialId: m.id,
        quantity: m.quantity,
        unit:
          (unitMap.get(m.id) as VectraMaterialUnit) ?? VectraMaterialUnit.PIECE,
      })),
    })
  }
  ...
  for (const item of newMaterials) {
    ...
    if (missing > 0) {
      await tx.vectraTechnicianMaterialDeficit.upsert({
        where: {
          technicianId_materialDefinitionId: {
            technicianId,
            materialDefinitionId: item.id,
