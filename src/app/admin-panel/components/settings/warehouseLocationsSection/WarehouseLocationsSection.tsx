...
import AddWarehouseLocationDialog from './AddWarehouseLocationDialog'
 ...
 */
const WarehouseLocationsSection = ({ title }: { title: string }) => {
  ...
  const deleteLocation = trpc.warehouse.deleteLocation.useMutation({
    onSuccess: async () => {
      ...
      await utils.warehouse.getAllLocations.invalidate()
    },
  ...
  return (
    <SettingsSection title={title}>
      ...
      <div className="flex justify-end mt-4">
        <AddWarehouseLocationDialog open={addOpen} onOpenChange={setAddOpen} />
