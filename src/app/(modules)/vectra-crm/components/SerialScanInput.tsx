...
interface Props {
  ...
  validStatuses?: VectraWarehouseStatus[]
...
const SerialScanInput = ({
   ...
   */
  const searchDevices = trpc.vectra.warehouse.searchDevices.useQuery(
    {
      query: value.trim(),
      allowedCategories,
    },
  ...
  const isAllowedForService = useCallback(
    (device: { name: string; category?: VectraDeviceCategory }): boolean => {
      ...
      switch (serviceType) {
        ...
        case 'ATV':
          return (
            category === VectraDeviceCategory.DECODER_1_WAY ||
  ...
  const fetchDevice = (serial: string) =>
    utils.vectra.warehouse.getBySerialNumber.fetch({
    ...
    try {
      ...
      if (localDevice) {
        ...
        if (
          ...
          allowedCategories[0] === VectraDeviceCategory.MODEM_GPON &&
          localDevice.category !== VectraDeviceCategory.MODEM_GPON
        ) {
          toast.warning(
