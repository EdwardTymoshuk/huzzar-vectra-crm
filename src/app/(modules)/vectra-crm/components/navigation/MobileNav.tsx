...
import WarehouseDropdownMenuMobile from '@/app/(modules)/vectra-crm/admin-panel/components/warehouse/warehouseLocalizations/WarehouseDropdownMenuMobile'
          ...
          return (
            <div key="warehouse" className="flex-1 flex items-stretch">
              {isTechnician ? (
                <Button
                  variant="ghost"
                  onClick={() => router.push(`${VECTRA_PATH}/?tab=warehouse`)}
                  className={cn(
        ...
        return (
          <Button
            ...
            onClick={() =>
              router.push(
                isTechnician
                  ? `${VECTRA_PATH}/?tab=${item.key}`
                  : `${VECTRA_PATH}/admin-panel?tab=${item.key}`
              )
            }
            className={cn(
              'flex flex-1 flex-col items-center justify-center text-sm sm:text-lg px-2 min-h-16 h-full py-4 rounded-none',
