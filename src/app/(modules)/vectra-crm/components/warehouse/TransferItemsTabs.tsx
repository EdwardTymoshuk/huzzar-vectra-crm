...
import WarehouseSelectedItemsPanel from '@/app/(modules)/vectra-crm/components/warehouse/WarehouseSelectedItemsPanel'
import {
  Tabs,
  TabsContent,
  TabsList,
...
const TransferItemsTabs = ({ technicianId, onClose }: Props) => {
  ...
  const { mutate: request, isLoading } =
    trpc.vectra.warehouse.requestTechTransfer.useMutation({
      onSuccess: () => {
