...
import { VectraOrderStatus, VectraTimeSlot } from '@prisma/client'
 ...
 *   mapped operator, mapped status, and a final VectraTimeSlot.
 */
...
/** Map file's status text → VectraOrderStatus enum (fallback PENDING) */
function mapStatus(raw: string): VectraOrderStatus {
  ...
  if (
    ...
    return 'NOT_COMPLETED' as VectraOrderStatus
  // Unknown → default
  return 'PENDING'
}
...
/** Convert "HH:MM-HH:MM" into VectraTimeSlot; snap to closest defined if not exact. */
function toTimeSlot(start: string, end: string): VectraTimeSlot {
  const key = `${start}-${end}`
