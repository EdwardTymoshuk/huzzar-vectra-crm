...
const ServiceConfigDialog = ({
  ...
  const selectedCategory: VectraDeviceCategory | undefined =
    primarySource === 'WAREHOUSE'
      ? primaryDevice?.category
      : clientCategory ?? undefined

  /** T-Mobile specific flags */
  const isPrimaryHfc = selectedCategory === VectraDeviceCategory.MODEM_HFC
  const isPrimaryGpon = selectedCategory === VectraDeviceCategory.MODEM_GPON
  ...
  const secondaryOptions = useMemo(() => {
    return devices.filter(
      (d) =>
        d.category === VectraDeviceCategory.MODEM_GPON &&
  ...
  /** Derived flags */
  const isDecoder2Way = selectedCategory === VectraDeviceCategory.DECODER_2_WAY
  const isModemGpon = selectedCategory === VectraDeviceCategory.MODEM_GPON
  ...
  const primaryOptions = useMemo(() => {
    const base = (() => {
      if (type === 'DTV') {
        ...
        return devices.filter(
          (d) =>
            d.category === VectraDeviceCategory.DECODER_1_WAY ||
            d.category === VectraDeviceCategory.DECODER_2_WAY
        )
      }
      ...
      if (type === 'NET') {
        ...
        return devices.filter(
          (d) =>
            d.category === VectraDeviceCategory.MODEM_HFC ||
            d.category === VectraDeviceCategory.MODEM_GPON
        )
      }
  ...
  const handleChangeSource = (src: VectraDeviceSource) => {
  ...
  const handleClientCategoryChange = (cat: VectraDeviceCategory | null) => {
    setClientCategory(cat)
    if (type === 'NET' && cat === VectraDeviceCategory.MODEM_GPON) {
      setDs('')
      setUs('')
    }
  }
  ...
  const isValid = useMemo(() => {
    ...
    if (
      canHaveRouter &&
      secondaryDevice &&
      secondaryDevice.category !== VectraDeviceCategory.MODEM_GPON
    ) {
      return false
    }
  ...
  const handleConfirm = () => {
    ...
    const payload: VectraActivatedService = {
          ...
          <div className="space-y-2 mb-6">
            ...
            <SerialScanInput
              ...
              onAddDevice={(dev) => {
                ...
                if (dev.category !== VectraDeviceCategory.MODEM_GPON) {
                  toast.warning(
              ...
              allowedCategories={[VectraDeviceCategory.MODEM_GPON]}
              variant="block"
            />
            {secondaryDevice && (
              <DeviceCard
              ...
              <>
                <SerialScanInput
                  ...
                  allowedCategories={[
                    ...
                    VectraDeviceCategory.NETWORK_DEVICE,
                  ]}
                />
