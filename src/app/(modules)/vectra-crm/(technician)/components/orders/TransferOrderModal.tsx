...
const TransferOrderModal = ({ open, orderId, onClose }: Props) => {
  ...
  const { data: technicians = [] } =
    trpc.vectra.user.getOtherTechnicians.useQuery({})
  const requestTransfer = trpc.vectra.order.requestTransfer.useMutation({
    onSuccess: () => {
      trpcUtils.vectra.order.getAssignedOrders.invalidate()
      onClose()
    },
  })
