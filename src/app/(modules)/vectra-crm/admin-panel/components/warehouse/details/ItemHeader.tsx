...
type DeviceCategoryKey = keyof typeof devicesTypeMap
...
const ItemHeader = ({ items, definition, activeLocationId = 'all' }: Props) => {
  ...
  const deviceCategoryLabel = (() => {
    ...
    if (definition.category in devicesTypeMap) {
      return devicesTypeMap[definition.category as DeviceCategoryKey]
    }
  ...
  return (
    <ItemStatsCard
      ...
      categoryOrIndex={isDevice ? deviceCategoryLabel : definition.index ?? '—'}
