 ...
 */
export async function writeOplReturnToOperatorReport(historyIds: string[]) {
  const history = await prisma.oplWarehouseHistory.findMany({
  ...
  sheet.mergeCells(rowCursor, 1, rowCursor, 6)

  titleRow.getCell(1).value = 'ZWROT DO OPERATORA'
