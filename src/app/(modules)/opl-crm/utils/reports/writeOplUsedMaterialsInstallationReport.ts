...
/**
 * writeOplUsedMaterialsInstallationReport
 * --------------------------------------------------------------------
 * Generates a monthly aggregated Excel report of all materials used
 * in INSTALLATION orders.
 */
export async function writeOplUsedMaterialsInstallationReport(
  year: number,
  month: number // 0–11
): Promise<Buffer> {
  ...
  const rows = await prisma.oplOrderMaterial.groupBy({
    by: ['materialId', 'unit'],
    where: {
      order: {
        type: 'INSTALLATION',
  ...
  const materials = await prisma.oplMaterialDefinition.findMany({
    where: {
