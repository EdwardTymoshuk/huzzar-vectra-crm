import {
  BORDER,
  LIGHT_GRAY,
} from '@/app/(modules)/opl-crm/lib/oplExelReportsConstants'
 ...
 */
export async function writeOplWarehouseStockReport(): Promise<Buffer> {
  const today = formatDate(new Date())

  // Fetch all stock on warehouse (not assigned to technicians)
  const allStock = await prisma.oplWarehouse.findMany({
    where: {
      status: 'AVAILABLE',
      assignedToId: null,
      locationId: { not: null },
  ...
  devTitle.value = `Stan magazynu — Urządzenia | ${today}`
  ...
  matTitle.value = `Stan magazynu — Materiały | ${today}`
