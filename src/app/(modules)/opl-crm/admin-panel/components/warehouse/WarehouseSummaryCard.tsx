//src/app/(modules)/opl-crm/admin-panel/components/warehouse/WarehouseSummaryCard.tsx
...
const WarehouseSummaryCard = () => {
  ...
  const { data, isLoading, isError } = trpc.opl.warehouse.getAll.useQuery(
  ...
  const devices = safeData.filter(
    (item) => item.itemType === 'DEVICE' && item.status === 'AVAILABLE'
  )
  const materials = safeData.filter(
