...
type TransferRow = {
  ...
  lines: LineRow[]
  status: OplLocationTransferStatus
  incoming: boolean
  createdAt: Date
}
...
const LocationTransfersTable = () => {
  ...
  const { data: incoming = [], isLoading: inLoading } =
    trpc.opl.warehouse.getIncomingLocationTransfers.useQuery(
      { locationId: activeLocationId || undefined },
      { staleTime: 30_000 }
    )

  const { data: outgoing = [], isLoading: outLoading } =
    trpc.opl.warehouse.getOutgoingLocationTransfers.useQuery(
      { locationId: activeLocationId || undefined },
      { staleTime: 30_000 }
    )

  const accept = trpc.opl.warehouse.confirmLocationTransfer.useMutation({
    onSuccess: () => {
      setLoadingId(null)
  ...
  const reject = trpc.opl.warehouse.rejectLocationTransfer.useMutation({
    onSuccess: () => {
      setLoadingId(null)
      utils.opl.warehouse.getIncomingLocationTransfers.invalidate()
  ...
  const cancel = trpc.opl.warehouse.cancelLocationTransfer.useMutation({
    onSuccess: () => {
      setLoadingId(null)
