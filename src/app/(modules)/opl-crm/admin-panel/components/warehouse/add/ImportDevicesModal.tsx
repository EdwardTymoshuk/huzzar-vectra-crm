...
const ImportDevicesModal = ({ open, onClose }: Props) => {
  ...
  const defsQuery = trpc.vectra.deviceDefinition.getAllDefinitions.useQuery(
    undefined,
    {
      enabled: open,
    }
  )
  const importMutation = trpc.vectra.warehouse.importDevices.useMutation()
    ...
    const run = async () => {
      ...
      try {
        ...
        if (identifiers.length > 0) {
          const result =
            await utils.vectra.warehouse.checkExistingIdentifiers.fetch({
              identifiers,
              locationId,
            })
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      await utils.vectra.warehouse.getAll.refetch()
      resetState()
      onClose()
    } catch (e) {
      console.error(e)
