...
import { OrderAssignmentVM } from '@/server/modules/opl-crm/helpers/mappers/mapOplOrderAssignmentToVM'
import { OplOrderListVM } from '@/server/modules/opl-crm/helpers/mappers/mapOplOrderToListVM'
...
import EditOrderModal from './EditOplOrderModal'
import OrderAccordionDetails from './OplOrderAccordionDetails'
...
export default OplOrdersTable

/* ============================ INNER TABLE ============================ */

const OplOrdersTableInner = ({
  searchTerm,
  readOnly,
  statusFilter,
  technicianFilter,
  ...
  const [editingOrder, setEditingOrder] = useState<OplOrderListVM | null>(null)
  const [isDeleteModalOpen, setIsDeleteModalOpen] = useState(false)
  const [orderToDelete, setOrderToDelete] = useState<OplOrderListVM | null>(
    null
  )
  ...
  const orders = useMemo<OplOrderListVM[]>(
    () => data?.orders ?? [],
    [data?.orders]
  )
  ...
  const deleteOrder = trpc.opl.order.deleteOrder.useMutation({
    onSuccess: () => {
      ...
      utils.opl.order.getRealizedOrders.invalidate()
    },
    onError: (err) => {
      ...
      <ConfirmDeleteDialog
        ...
        onConfirm={async () => {
          ...
          try {
            ...
            console.error('[OplOrdersTable delete error]', err)
          }
          setIsDeleteModalOpen(false)
        }}
