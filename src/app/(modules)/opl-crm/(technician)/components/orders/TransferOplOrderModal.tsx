...
const TransferOplOrderModal = ({ open, orderId, onClose }: Props) => {
  const [technicianId, setTechnicianId] = useState<string | null>(null)

  const trpcUtils = trpc.useUtils()

  const { data: technicians = [] } = trpc.opl.user.getOtherTechnicians.useQuery(
    {}
  )
  const requestTransfer = trpc.opl.order.requestTransfer.useMutation({
    onSuccess: () => {
...
export default TransferOplOrderModal
