...
const OplItemHeaderTech = ({ items }: Props) => {
    ...
    if (isDevice) {
      ...
      return items.reduce((total, item) => {
        const hasUsedOnOrder = item.history.some(
          (h) =>
            h.action === OplWarehouseAction.ASSIGNED_TO_ORDER &&
    ...
    return items.reduce((total, item) => {
      const usedForOrders = item.history
        .filter(
          (h) =>
            h.action === OplWarehouseAction.ASSIGNED_TO_ORDER ||
            (h.action === OplWarehouseAction.ISSUED &&
              h.assignedOrderId !== null &&
              h.performedBy?.id === techId &&
              (h.quantity ?? 0) > 0)
        )
  ...
  return (
    <ItemStatsCard
      ...
      categoryOrIndex={
        isDevice
          ? first.category
            ? oplDevicesTypeMap[first.category]
            : '—'
          : first.index ?? '—'
      }
...
export default OplItemHeaderTech
