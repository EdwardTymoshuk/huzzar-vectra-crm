...
import { mapTechnicianTransferToVM } from '@/server/modules/opl-crm/helpers/mappers/TechnicianTransferRowVM'
import { mapTechnicianStockToOutgoingTransferVM } from '@/server/modules/opl-crm/helpers/mappers/mapTechnicianStockToOutgoingTransferVM'
...
const OplTechnicianTransfersTable = () => {
  const [cancelLoadingId, setCancelLoadingId] = useState<string | null>(null)
  const [acceptingId, setAcceptingId] = useState<string | null>(null)
  const [rejectingId, setRejectingId] = useState<string | null>(null)

  const { data: incoming = [], isLoading: incLoading } =
    trpc.opl.warehouse.getOplIncomingTechTransfers.useQuery(undefined, {
      staleTime: 30_000,
    })

  const { data: myStock = [], isLoading: stockLoading } =
    trpc.opl.warehouse.getTechnicianStock.useQuery({ technicianId: 'self' })

  const utils = trpc.useUtils()

  const accept = trpc.opl.warehouse.confirmOplTechTransfer.useMutation({
    onSuccess: () => {
      setAcceptingId(null)
      utils.opl.warehouse.invalidate()
  ...
  const reject = trpc.opl.warehouse.rejectOplTechTransfer.useMutation({
    onSuccess: () => {
      setRejectingId(null)
  ...
  const cancel = trpc.opl.warehouse.cancelOplTechTransfer.useMutation({
    onSuccess: () => {
        ...
        const itemDetails = isDevice ? (
          <>
            <span className="font-semibold break-all">
              {row.category ? oplDevicesTypeMap[row.category] : ''} {row.name}
            </span>
...
export default OplTechnicianTransfersTable
