...
type Props = {
  setCategoryFilter: (category: OplDeviceCategory | null) => void
}

type CategoryFilterValue = 'all' | OplDeviceCategory

/**
 * OplWarehouseFilterTech
 * ------------------------------------------------------
 ...
 */
const OplWarehouseFilterTech = ({ setCategoryFilter }: Props) => {
  ...
  const { data, isLoading } = trpc.opl.warehouse.getTechnicianStock.useQuery({
    technicianId: 'self',
  })
  ...
  const availableCategories = useMemo(() => {
    if (!data) return []
    const unique = new Set<OplDeviceCategory>()
    data.forEach((item) => {
...
export default OplWarehouseFilterTech
