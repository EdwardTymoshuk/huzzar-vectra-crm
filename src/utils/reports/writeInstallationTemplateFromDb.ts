...
/**
 * Generates a monthly installation report for a specific technician.
 * Uses the same expandedRows already generated by writeInstallationTemplateFromDb.
 * Removes all non-technician rows while keeping the template, styles and formulas.
 */
export async function writeInstallationTemplateForTechnician(
  year: number,
  month: number,
  technicianId: string
): Promise<Buffer> {
  // Generate FULL workbook for this year (including expandedRows)
  const fullBuffer = await writeInstallationTemplateFromDb(year)
  ...
  /**
   * We must fetch orders for this month AND this technician,
   * to know which rows to keep in the final Excel sheet.
   */
  const technicianOrders = await prisma.order.findMany({
    where: {
  ...
  const ORDER_NUM_COL = COLS.indexOf('schemat / nr klienta') + 1

  // Remove all rows that DO NOT belong to this technician’s orders
  const maxRows = ws.rowCount
  for (let r = DATA_START_ROW; r <= maxRows; r++) {
    const orderNumber = ws.getCell(r, ORDER_NUM_COL).value?.toString() ?? ''

    // Order numbers are unique, so we can match them
    const belongs = technicianOrders.some((o) => o.orderNumber === orderNumber)

    if (!belongs) {
      ws.getRow(r).values = [] // clear row visually
    }
  }

  // Optional: compact empty rows (if needed)
  ws.spliceRows(DATA_START_ROW, 0)
